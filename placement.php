<?php require "common/header.php" ?>
<style>
    .academics-objective {
        border-top: 5px solid #f56e10;
        border-top-left-radius: 5px;
        border-top-right-radius: 5px;
    }

    .card.h-100 {
        margin-bottom: 10px;
    }

    .border-1 {
        box-shadow: 0 0 10px rgba(0, 0, 0, 0.3);
        margin: 5px;
    }

    .h-100 {
        height: 98% !important;
    }
</style>
<div class="banner-area about" style="background-image: url(assets/images/banner/47.webp);">
    <div class="d-table">
        <div class="d-table-cell">
            <div class="container">
                <div class="page-title-content">
                    <h1>Placements <br>at GNC</h1>
                </div>
            </div>
        </div>
    </div>
</div>
<section class="placementVideo-section pt-5 pb-5">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-8">
                <div class="programVideo">
                    <div class="gdlr-core-fluid-video-wrapper">
                        <iframe src="https://www.youtube.com/embed/7hk7G0SEhZ0?autoplay=1&amp;mute=1&amp;playlist=7hk7G0SEhZ0&amp;loop=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen="" id="gdlr-video-561164" width="560" height="315" style="filter: drop-shadow(4px 4px 5px black);">
                        </iframe>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="placementVideo-section__content">
                    <div class="page-heading">
                        <h2>
                            See how Guru Nanak College of Pharmaceutical Sciences students prepare and bag
                            <span style="color: orangered;">Rs. 1.01 CR placements </span>
                            at top companies!
                        </h2>
                    </div>
                    <p style="color: black;">A whopping 100+ Corporates visited for Placements &amp; Internship in the current academic session.</p>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="visionMission-section pt-5 pb-5">
    <div class="container">
        <div class="row">
            <div class="col-lg-6">
                <div class="page-sub-heading">
                    <h3>Vision:</h3>
                </div>
                <p style="color: black;">At Guru Nanak College of Pharmaceutical Sciences, Dehradun, our vision for placements is focused on empowering students with the necessary skills and opportunities for a seamless transition into their professional careers. We prioritize practical, industry-relevant training and maintain strong industry linkages to provide diverse placement opportunities. Our approach is inclusive, ensuring every student has access to our comprehensive placement services. From skill development workshops to final job interviews, we offer end-to-end support, preparing students for successful careers both locally and globally.</p>
                <br>
                <div class="page-sub-heading">
                    <h3>Our Best Practices:</h3>
                </div>
                <ul class="list-unstyled ps-4 list-icon-4 mb-3">
                    <li><b>Flexible Training Modules:</b> Ensuring that training materials can easily transition between offline and online formats to accommodate various learning environments.</li>
                    <li><b>Seamless Integration:</b> The ability to switch from online to offline (and vice versa) without disrupting the learning process.</li>
                    <li><b>Student-Centric Placement Process:</b> Policies and procedures designed with the student's needs and preferences in mind.</li>
                    <li><b>Adaptable Learning Strategies:</b> Catering to diverse learning styles and preferences, whether in a physical classroom or a virtual setting.</li>
                </ul>
            </div>
            <div class="col-lg-6">
                <div class="page-sub-heading">
                    <h3>Student Learn to:</h3>
                </div>
                <ul class="list-unstyled ps-4 list-icon-4 mb-3">
                    <li><b>Resume and Cover Letter Writing:</b> Techniques for creating impactful resumes and cover letters.</li>
                    <li><b>Interview Skills:</b> Strategies for successful job interviews, including practice sessions.</li>
                    <li><b>Job Search Strategies:</b> Effective methods for finding and applying to jobs.</li>
                    <li><b>Career Guidance:</b> Advice on career paths and understanding different industries.</li>
                    <li><b>Networking Skills:</b> How to build and utilize professional networks.</li>
                    <li><b>Workplace Etiquette:</b> Learning professional behavior and communication.</li>
                    <li><b>Access to Opportunities:</b> Information on internships and job openings.</li>
                    <li><b>Industry Insights:</b> Knowledge about current trends and employer expectations.</li>
                    <li><b>Soft Skills Development:</b> Enhancing teamwork, leadership, and problem-solving skills.</li>
                    <li><b>Personal Branding:</b> Creating a professional online presence.</li>
                </ul>
            </div>
        </div>
    </div>
</section>
<?php require "company-logo.php" ?>
<section class="container">
    <h4>Training & Placement Committee</h4>
    <table class="table table-bordered">
        <thead>
            <tr class="heading">
                <th scope="col"><b>S.No.</b></th>
                <th scope="col"><b>Name</b></th>
                <th scope="col"><b>Designation</b></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th scope="row">1</th>
                <td>Mr. Sunil Kumar Malik</td>
                <td>Chairperson </b></td>
            </tr>
            <tr>
                <th scope="row">2</th>
                <td>Dr. Nidhi Chatterjee</td>
                <td>Placement Executives</b></td>
            </tr>
            <tr>
                <th scope="row">3</th>
                <td>Dr. Kashif Hussain</td>
                <td>Faculty In-charge</b></td>
            </tr>
        </tbody>
    </table>
</section>
<section class="placementSupport-section position-relative mt-5 mb-5">
    <div class="container p-0">
        <div class="row g-0">
            <div class="col-lg-5 d-flex align-items-end position-relative ps-5 pb-5" style="background-image: url('assets/images/placement-img.webp'); background-size: cover;">
                <div class="main-heading text-white mb-0 position-relative">
                    <h2 class="mb-3">The support your <br>journey needs!</h2>
                    <p style="color: #ffffff;"> Guru Nanak College of Pharmaceutical Sciences has come a<br> long way since its inception providing world-class education.</p>
                </div>
            </div>
            <div class="col-lg-7">
                <div class="choose-section__content">
                    <div class="row g-0">
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Internships and On-the-job Trainings</h4>
                                    <p style="color: black !important;">During your course of studies, you will have ample opportunities to gain industry experience through internships and on-the-job training.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Corporate Connect Program</h4>
                                    <p style="color: black !important;">We have established the Career Competency Program (CCP) at Guru Nanak College of Pharmaceutical Sciences with the aim of boosting our graduates' job prospects and creating an environment that attracts a greater number of multinational corporations (MNCs).</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Career Resource Centre</h4>
                                    <p style="color: black !important;">Empowerment comes through awareness. At Guru Nanak College of Pharmaceutical Sciences, the Career Resource Centre is dedicated to enlightening, linking, and offering guidance to students regarding placement prospects.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Accreditations &amp; Validations</h4>
                                    <p style="color: black !important;">Accreditations and validations stand as the foundational pillars of accuracy and trustworthiness, playing a pivotal role in attracting multinational corporations to our campus for recruitment purposes.</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Placement Cell</h4>
                                    <p style="color: black !important;">GNC's Placement Cell is available year-round to facilitate your connections with leading multinational corporations (MNCs).</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-xxl-4 col-sm-6">
                            <div class="card h-100 border-1" style="filter: drop-shadow(3px 3px 4px black);">
                                <div class="card-body academics-objective">
                                    <h4 style="color: black !important;">Foreign Internships &amp; Semester Exchange</h4>
                                    <p style="color: black !important;">Participating in Global Internships and Semester Exchange programs enhances your marketability by providing you with a unique blend of international exposure, hands-on experience, and a wealth of knowledge and skills acquired on the job.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section>
    <h2>Placements</h2>
    <div class="container">
<div class="button-container">
        <button id="toggle2024Button">2024 Batch</button>
        <button id="toggle2023Button">2023 Batch</button>
        <button id="toggle2022Button">2022 Batch</button>
    </div>
    <div class="batch-list">
        <div class="table-container">
            <table id="batch2024Table" style="display: none;" class="table table-bordered" style="filter: drop-shadow(3px 3px 4px black);">
                <thead>
                    <tr class="heading">
                        <th scope="col"><b>Year</b></th>
                        <th scope="col"><b>Name of student who has been placed</b></th>
                        <th scope="col"><b>Program graduated from</b></th>
                        <th scope="col"><b> Year of graduation</b></th>
                        <th scope="col"><b>Name of the employer</b></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>2024</td>
                        <td>Alok Pandey</td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Sheena Bhatt</td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>ANKUL KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Arun Thakur </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>MOHD AFSHAN </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>MOHD SHAHMEER REHMAN </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>RAHUL KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>NITISH KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Anit Kumar </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Charanjeet Singh </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Pal Sachin Bhullur </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Pranjal Prajapati </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Shenu Prasad Ram </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Prince Kumar </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>Windlas Biotech Limited </td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>SATYAM KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>Swarnimaksh Life Sciences Pvt Ltd </td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Sujit Kumar </td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>Swarnimaksh Life Sciences Pvt Ltd </td>
                    </tr>
                    <tr>
                        <td>2024</td>
                        <td>Saif Ali</td>
                        <td>B.Pharm</td>
                        <td>2024</td>
                        <td>Zydus</td>
                    </tr>
                </tbody>
            </table>
        </div>

        <div class="table-container">
            <table id="batch2023Table" style="display: none;" class="table table-bordered" style="filter: drop-shadow(3px 3px 4px black);">
                <thead>
                    <tr class="heading">
                        <th scope="col"><b>Year</b></th>
                        <th scope="col"><b>Name of student who has been placed</b></th>
                        <th scope="col"><b>Program graduated from</b></th>
                        <th scope="col"><b> Year of graduation</b></th>
                        <th scope="col"><b>Name of the employer</b></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>2023</td>
                        <td>AATIF SIDDIQUI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Virtual Physician Scribe </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>ABBAS ALAM </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Pathkind lab </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>ADITI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Virtual Physician Scribe </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td> Aditya Kumar </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>ALOK KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Ved lifesavers </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td></td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td></td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>AMIT BISWAS </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Virtual Physician Scribe </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>ANAND KUMAR PRAJAPATI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Windlas Biotech Limited </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td> Anil Adhikari </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>ATUL SHARMA </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>BHAVESH BHARTI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Rhydburg Pharmaceuti cals Ltd </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>Faraz Iqrar </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>JAYITA BACHHAR </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>IDS Infotech Ltd </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>MANTU KUMAR JAISWAL </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Hetero Healthcare Limited </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>MD GAZNEE </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>MENUKA CHAUDHARY </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>Mohd Aalam </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Sunpharma </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>NEELMANI KASHYAP </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Virtual Physician Scribe </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>RANJAN RAJ </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>WINDLAS BIOTECH LIMITED </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>RASIKA </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>Sandeep Singh </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Neclife </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>SHAHID ALI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Neclife </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>SOURAV DHIBAR </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>SOURAV KUMAR </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>SUDANSHU SHARMA </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>A R C E L I L I F E S C I E N C E P V T L T D </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>TANSHAVI CHAUHAN </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Virtual Physician Scribe </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>USMAN ALI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>Zeeshan </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Praksahdeep Hospital </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>NAVEEN RAWAT </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2023</td>
                        <td>RAUNAK KUMAR PRAJAPATI </td>
                        <td>B.Pharm</td>
                        <td>2023</td>
                        <td>Pathkind lab </td>
                    </tr>
                </tbody>
            </table>
        </div>

        <div class="table-container">
            <table id="batch2022Table" style="display: none;" class="table table-bordered" style="filter: drop-shadow(3px 3px 4px black);">
                <thead>
                    <tr class="heading">
                        <th scope="col"><b>Year</b></th>
                        <th scope="col"><b>Name of student who has been placed</b></th>
                        <th scope="col"><b>Program graduated from</b></th>
                        <th scope="col"><b> Year of graduation</b></th>
                        <th scope="col"><b>Name of the employer</b></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>2022</td>
                        <td>Amaan ansari </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>lupin </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Amit kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>Indivirtus AB7 Scribing & RCM Pvt. Ltd. </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Chandan kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>Drug Store </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Chinmay goyel </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>Unimedicos </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Iainehskhem lyngkhoi </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Kunal upmanyu </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Mohd Akram </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>Mankind </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Mohd Qamar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Mohd sarim irfan </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Mohit Kumar Modanwal </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Motin </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>CBR Hospital </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Nayeem Ahmad Sheikh </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Nikita Rawat </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Rajan Kushwaha </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>pontika Aerotech limited </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Rohit Giri </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Sachin Kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Sadam yousuf rather </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>SD promo media </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Sambhav Pathak </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Satish Kumar Yadav </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Shubham Kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE</td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Vikash kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>Dr. laal hospital </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Vipin Kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Sunil Kumar </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                    <tr>
                        <td>2022</td>
                        <td>Ayush Sati </td>
                        <td>B.Pharm</td>
                        <td>2022</td>
                        <td>NECLIFE </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
    </div>
</section>
<?php require "testimoni.php" ?>
<?php require "common/footer.php" ?>

<style>
   .button-container {
    text-align: center; /* Center the button container */
    margin-top: 20px;
    margin-bottom: 20px;
}

.button-container button {
    padding: 10px 20px;
    font-size: 16px;
    margin: 0 10px; /* Space between buttons */
    cursor: pointer;
    display: inline-block;
    background-color: #1ebeff ;
    color: white;
}
.button-container button:hover{
    background-color:#f56e10
}
    .batch-list {
        text-align: center;
    }
    h2{
        text-align: center;
    }
</style>

<script>
    document.getElementById('toggle2024Button').addEventListener('click', function() {
        const batch2024Table = document.getElementById('batch2024Table');
        if (batch2024Table.style.display === 'none') {
            batch2024Table.style.display = 'table';
            this.textContent = '2024 Batch';
        } else {
            batch2024Table.style.display = 'none';
            this.textContent = '2024 Batch';
        }
    });

    document.getElementById('toggle2023Button').addEventListener('click', function() {
        const batch2023Table = document.getElementById('batch2023Table');
        if (batch2023Table.style.display === 'none') {
            batch2023Table.style.display = 'table';
            this.textContent = '2023 Batch';
        } else {
            batch2023Table.style.display = 'none';
            this.textContent = '2023 Batch';
        }
    });

    document.getElementById('toggle2022Button').addEventListener('click', function() {
        const batch2022Table = document.getElementById('batch2022Table');
        if (batch2022Table.style.display === 'none') {
            batch2022Table.style.display = 'table';
            this.textContent = '2022 Batch';
        } else {
            batch2022Table.style.display = 'none';
            this.textContent = '2022 Batch';
        }
    });
</script>

<thead>
    <tr class="heading">
        <th scope="col"><b>Year</b></th>
        <th scope="col"><b>Name of student who has been placed</b></th>
        <th scope="col"><b>Program graduated from</b></th>
        <th scope="col"><b>Year of Graduation</b></th>
        <th scope="col"><b>Name of the Employer</b></th>
    </tr>
</thead>
<tbody>
    <tr>
        <td>2024</td>
        <td>ALOK PANDEY</td>
        <td>B.Pharm</td>
        <td>2024</td>
        <td>NECLIFE</td>
    </tr>
</tbody>